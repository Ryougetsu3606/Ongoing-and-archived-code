...
from myalgody import dy_bilevel_kd
...
def main(args):
    ...
    print(f'mission:KD-decums, lr: {LR}, metalr:{META_LR}, batch_size: {BATCH_SIZE}, student_size: {args.student_size}, algorithm: {args.algorithm}, opt: {args.opt}')
    ...
    # saving
    save_path = f'./checkpoints/KD-decums-dykdtetsr/{LR}_{args.student_size}_{args.algorithm}_{args.opt}'
    ...
    class AlphaScheduler(nn.Module):
        def __init__(self, embedding_dim=8, hidden_dim=16):
            ...
            # 2. 一个超小的2层MLP，将 e 映射为 alpha_logits
    ...
    a_sche.to(DEVICE)
    optimizer = [torch.optim.AdamW(a_sche.parameters(), lr=META_LR), torch.optim.AdamW(student_model.parameters(), lr=LR)]
    scheduler = [torch.optim.lr_scheduler.StepLR(optimizer[0], step_size=25, gamma=0.5), torch.optim.lr_scheduler.StepLR(optimizer[1], step_size=25, gamma=0.5)]
    for epoch in range(EPOCHS):
        st = time.time()
        train_loss, train_lower_loss, train_upper_loss = dy_bilevel_kd(a_sche, student_model, teacher_model, train_loader, val_loader,optimizer, epoch, DEVICE, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algorithm', type=str, default='penalty')
